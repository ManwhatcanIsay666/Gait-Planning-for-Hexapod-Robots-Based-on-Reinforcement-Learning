...
class HexapodExpert:
    ...
    def __init__(self,L1,L2,L3):
        ...
        self.tripod_gait = {"rf":0,"rm":0.5,"rr":0,"lf":0.5,"lm":0,"lr":0.5}
        self.init_flag = True
    ...
    def get_next_theta(self,observation):
        """
        * @brief 根据模型足尖位置返回下一步位置
        * @param observation是一个18维向量，分别为当前各关节的角度，按三个三个按顺序排列
        ...
        * @warning None
        ...
        target_angle = []
        id = 0
        for _ in self.joint_name_to_id:
            now_position = self.forward_kinematics(observation[id*3:3+id*3])
            new_pause = remainder(atan2(now_position[2] - self.init_position[2], now_position[1] - self.init_position[1]) + self.phase_step,2*pi)
            if new_pause > 0:    #摆动相
                x = self.init_position[0] - 0.02
                y = self.init_position[1] + self.move * cos(new_pause)
                z = self.init_position[2] + self.high * sin(new_pause)
            else:                 #支撑相
                x = self.init_position[0] - 0.02
                y = self.init_position[1] + self.move * cos(new_pause)
...
if __name__ == "__main__":
    ...
    while True:
        target_angle = hexapod.get_next_theta(hexapod.get_leg_posture()[:18])
        tempid = 0
