...
L2 = 0.0661
L3 = 0.08
model_name = "2026-09-13 11_59_55_400000_steps"
...
class CustomEnv(gym.Env):
    ...
    def step(self, action):
        ...
        reward = (-0.01*sum(abs(x) for x in self.state[36:54]) +                                                         #力矩惩罚
                  #-0.3*((4*self.state[-8])**2+(3*abs(self.state[-7]))**2)                                               #欧拉角倾斜惩罚
                  +4*(1/(abs((cos(self.state[-2])*self.state[-5]+sin(self.state[-2])*self.state[-4])-0.5)+1)-2/3)   #速度奖励                                    #速度奖励
                  -0.2*sqrt(abs(atan2(self.state[-4], self.state[-5]) - self.state[-2]))                          #速度方向惩罚
                  -0.01                                                                                                    #存活奖励
                  -0.3*sqrt(abs(self.state[-6]-self.state[-2]))                                                         #朝向惩罚
                  )
...
if __name__ == '__main__':
    ...
    model.learn(total_timesteps=500000,log_interval=1,callback=checkpoint_callback,reset_num_timesteps=False)
